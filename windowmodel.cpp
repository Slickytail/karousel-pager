...
QVariant WindowModel::data(const QModelIndex &index, int role) const
{
    if (role == AbstractTasksModel::Geometry) {
        ...
        // Deliberately do not clamp to the desktop/screen rect. Karousel places
        // windows that are scrolled out of the viewport just outside the screen,
        // so the pager needs their raw geometry in order to draw them.
        return windowGeo;
